<%@ page import="com.laodeng.crm.settings.domain.User" %>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#addBtn").click(function(){
			...
			$.ajax({
				...
				success : function(data){
					...
					// 遍历出来的每一个e，就是每一个user对象
					$.each(data, function(i, e){
						// 获取所有者下拉框
						if(e.id == id){
							$("#create-owner").append("<option value="+ e.id +" selected>"+ e.name +"</option>");
						}else{
							$("#create-owner").append("<option value="+ e.id +">"+ e.name +"</option>");
						}
					})

					// 所有者下拉列表框处理完之后，展示模态窗口
				},
				dataType : "json"
			})
						...
						* */
						...
						/*
                            注意：
                                ...
                                但是表单的jQuery对象，没有为我们提供reset()方法，让我们重置表单（坑，IDEA为我们提示了reset()方法）

                                虽然jQuery对象没有为我们提供了reset方法，但是原生js为我们提供了reset方法
                                我们要将jQuery对象转换为原生js对象dom对象
		...
		$("#deleteBtn").click(function(){
			...
			if($xz.length == 0){
				...
				if(window.confirm("确定删除所选中的记录吗？")){

					// url : "workbench/activity/delete.do?id=xxx&id=xxx&id=xxx",
					// 拼接参数
					var param = "";
					// 将$xz中每一个dom对象遍历出来，取其value值，就相当于取得了需要删除的记录的id值
		...
		/*$("#qx").click(function(){
			...
			if($("#qx")[0].checked){
				...
				// 如果全选将要被选中
				// 将其它复选框全都选中
				var checkBoxDomArray = $(".epx");
				// 获取市场活动所有复选框的jquery对象(数组)
				$.each(checkBoxDomArray, function(i, n){
		...
		*/
		...
		$("#searchBtn").click(function(){
			...
			/*
                点击查询按钮的时候，我们应该将搜索框中的信息保存起来，保存到隐藏域当中
             */
	...
	/*
		...
		pageList方法，就是发出ajax请求到后台，从后台取得最新的市场活动列表数据
						通过响应回来的数据，局部刷新市场活动信息列表

		我们都在哪些情况下，需要调用pageList方法（什么情况下需要刷新一下列表呢？）
			1、点击左侧菜单中的“市场活动”超链接
			2、添加、修改、删除后需要出刷新市场活动列表
			3、点击查询按钮的时候
			4、点击分页组件的时候

			以上为pageList方法制定了六个入口，也就是说：在以上6个操作执行完毕后，
			我们必须要调用pageList方法，刷新市场活动列表
		...
		$.ajax({
			...
			success: function (data) {
				...
				$.each(data.dataList, function (i, e) {
					...
					html += '<td><input type="checkbox" name="xz" class="epx" value=' + e.id + ' /></td>';
					...
					<form class="form-horizontal" role="form">
						...
						<div class="form-group">
							...
							<div class="col-sm-10" style="width: 81%;">
								<!--
									关于文本域textarea
										1、标签对一定是要以标签对的形式来呈现，正常状态下，标签对要紧紧地挨着
